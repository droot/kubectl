...
func (rv *refvarTransformer) Transform(resources resource.ResourceCollection) error {
	...
	for GVKn := range resources {
		obj := resources[GVKn].Data
		objMap := obj.UnstructuredContent()
		for _, pc := range rv.pathConfigs {
			if !types.SelectByGVK(GVKn.GVK, pc.GroupVersionKind) {
				continue
			}
